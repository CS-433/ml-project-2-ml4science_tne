...
\subsection{Analysis of frequency bands}
As previoulsy with the action recognition task, the models was trained with features extracted from each frequency bands for both execution (see \aref{appfig:accuracies_ex_freq_bands}) and observation (see \aref{appfig:accuracies_obs_freq_bands}). The results are quite different from the action recognition task. The models are not able to capture the differences between the two types of movements, with accuracies barely above chance level. This suggests that the features extracted from the EEG signals are not able to capture the differences between the two types of movements.
...
\subsection{Results}
TODO FLORIAN
The architectures of the CNNs used are detailed in \aref{subsec:cnnarchi}.

% Both types of CNN achieve high performance on the action recognition task (). However, their performance are unfortunately lacking in the two other movement recognition tasks. The results are however encouraging, and more work should be done on finding good CNNs for analysing such data.
...
\paragraph{Analysis of accuracy by frequency bands}
